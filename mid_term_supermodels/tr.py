"""Lettura del file TR (Take Rate): mix dei modelli e scelte degli optional.

In parole semplici: il TR indica, in percentuale, quanto spesso i clienti
scelgono ogni modello e ogni optional. Questo modulo lo legge dagli Excel mensili
e prepara i dati usati dalla simulazione Monte Carlo. Gestisce anche
l'"affidabilita'" del TR, che regola quanto la simulazione segue i dati dichiarati.
"""
import json
import os
import pandas as pd
...
# ---------------------------------------------------------------------------
def _resolve_affidabilita_path() -> str:
    """Trova affidabilita_config.json in modo robusto, indipendente da CWD e
    da dove e' collocato il package. Candidati in ordine:
      1. <package>/Input/affidabilita_config.json   (layout canonico: Input dentro mid_term)
      2. <padre-del-package>/Input/...              (layout legacy: Input accanto a mid_term)
    Ritorna il primo esistente, altrimenti (1) come default (warning + fallback)."""
    here = os.path.dirname(os.path.abspath(__file__))
    candidates = [
        os.path.join(here, 'Input', 'affidabilita_config.json'),
        os.path.join(os.path.dirname(here), 'Input', 'affidabilita_config.json'),
    ]
    for c in candidates:
        if os.path.isfile(c):
            return c
...
_AFFIDABILITA_CONFIG_PATH = _resolve_affidabilita_path()
...
class ModelMix:
    ...
    def validate(self):
        """Controllo di coerenza: verifica che i take rate dei modelli sommino a
        ~1 (tolleranza 0.99-1.01); logga un warning se fuori range."""
