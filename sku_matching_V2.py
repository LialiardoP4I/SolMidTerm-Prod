...
def load_lead_times(filepath: str = '.\\Input\\quantity e residual.xlsx') -> pd.DataFrame:
    """
    Carica i lead time dei componenti dal file Excel.

    Il lead time indica quanti mesi prima bisogna ordinare un componente.
    È fondamentale per calcolare il safety stock corretto.

    Formato file atteso:
    - Header alla riga 3 (header=2)
    - Colonna "Codice componenti": codice SKU
    - Colonna "Residual Purchasing Time": lead time in mesi
    - Colonna "Somma di Qtà comp.": quantità per moto
    - Colonna "Testo breve oggetto": descrizione
    ...
    """
    print(f"\n{'='*60}")
    print(f"CARICAMENTO LEAD TIME")
    print(f"{'='*60}")

    df = pd.read_excel(filepath, header=0)

    # Nomi colonne nel file
    sku_col = 'Codice componenti'
    lt_col = 'Residual Purchasing Time (months), quanto rimane di mesi rispetto al frozen period'
    qty_col = 'Somma di Qtà comp. (UMC)'
    desc_col = 'Testo breve oggetto'

    # Estrai solo colonne necessarie
    lead_times = df[[sku_col, lt_col, qty_col, desc_col]].copy()
    lead_times.columns = ['SKU', 'Lead_Time_Months', 'Quantity_Per_Bike', 'Description']

    # Rimuovi righe senza lead time
    lead_times = lead_times.dropna(subset=['Lead_Time_Months'])

    # Mantieni lead time decimale (interpolazione lineare nel matching)
    # Se serve 2.3 mesi, aggreghiamo 2 mesi + 0.3 del mese 3
    lead_times['Lead_Time_Months_Ceil'] = lead_times['Lead_Time_Months'].copy()

    # Default quantità = 1 se non specificata
    lead_times['Quantity_Per_Bike'] = lead_times['Quantity_Per_Bike'].fillna(1.0)

    # Filtra: solo lead time > 0 (includi anche LT < 1 mese, ad es. 0.25)
    lead_times = lead_times[lead_times['Lead_Time_Months_Ceil'].notna()]
    lead_times = lead_times[lead_times['Lead_Time_Months_Ceil'] > 0]

    # Aggrega per SKU (stesso SKU può apparire più volte)
    # Prendi max lead time, media quantità, prima descrizione
    lead_times_unique = lead_times.groupby('SKU', as_index=False).agg({
        'Lead_Time_Months': 'max',
        'Lead_Time_Months_Ceil': 'max',
        'Quantity_Per_Bike': 'mean',
        'Description': 'first'
    })

    print(f"SKU univoci con lead time: {len(lead_times_unique)}")
    return lead_times_unique
